...
class XButtonsDriver():
    ...
    def __init__(self, i2c_bus: SMBus, key_pressed_callback: callable, key_released_callback: callable):
        ...
        self.page_button_states = [0, 0]
        self.page_button_states_old = [0, 0]
    ...
    def check_for_change(self):
        ...
        self.i2c_bus.write_byte_data(self.multiplexer_1, GPIOB, 0xDF)  # set all rows high
        ...
        for row in range(len(rows)):
            ...
            raw_row_button_states_b = [0, 0, 0, 0, 0, 0, 0, 0]
            for bit in range(0, 8):
                raw_row_button_states_b[bit] = (self.i2c_bus.read_byte_data(self.multiplexer_1, GPIOB) >> bit) & 1
                
            row_button_states_b = raw_row_button_states_b[0:5]
            
            self.page_button_states = raw_row_button_states_b[6:8]    
        ...
        # check page up and down buttons
        #reverse the states
        for i in range(0, 2):
            if self.page_button_states[i] == 0:
                self.page_button_states[i] = 1
            else:
                self.page_button_states[i] = 0
        
        key_counter: int = 30        
        for digit in range(len(self.page_button_states)):
            if self.page_button_states[digit] != self.page_button_states_old[digit]:
                if self.page_button_states[digit] == 1:
                    self.key_pressed_callback(key_counter)
                else:
                    self.key_released_callback(key_counter)
            key_counter += 1
        ...
        for digit in range(len(self.page_button_states)):
            self.page_button_states_old[digit] = self.page_button_states[digit]
