...
class RotaryEncoder(object):
    # 100 Hz for external timer frequency as default for debouncing the push button of the rotary encoder
    PUSHBUTTON_EXTERNAL_TIMER_FRQUENCY_DEFAULT: int = 100
    # 30 ms debounce time as default for push button of rotary encoder
    PUSHBUTTON_DEBOUNCE_TIME_DEFAULT: int = 30
    ...
    # integrated signal for debouncing the push button signal (see http://www.kennethkuhn.com/electronics/debounce.c for details)
    pushbutton_integrator: int
    # max value possible for integrated signal for debouncing the push button signal
    pushbutton_integrator_max: int
    ...
    # number of GPIO (not pin number!) for signal of pushbutton
    gpio_number_signal_pushbutton: int
    # callback called, if encoder is rotated clockwise
    rotated_clockwise_callback: Callable
    # callback called, if encoder is rotated counterclockwise
    rotated_counterclockwise_callback: Callable
    # callback called, if pushbutton is pressed
    pushbutton_pressed_callback: Callable
    # callback called, if pushbutton is released
    pushbutton_released_callback: Callable
    # user defined data belonging to this instance of the RotaryEncoder class
    instance_data: object
    # external timer frequency for debouncing the push button signal
    pushbutton_external_timer_frequency: int
    ...
    def __init__(self, gpio_number_signal_a: int,
        ...
        self.pushbutton_integrator_max = (
            pushbutton_debounce_time / 1000) / pushbutton_external_timer_frequency
    ...
    def setup(self) -> None:
        GPIO.setup(self.gpio_number_signal_a,
                   GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.setup(self.gpio_number_signal_b,
                   GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.setup(self.gpio_number_signal_pushbutton,
                   GPIO.IN, pull_up_down=GPIO.PUD_UP)

        GPIO.add_event_detect(self.gpio_number_signal_a,
                              GPIO.BOTH, callback=self._a_changed_callback)
        GPIO.add_event_detect(self.gpio_number_signal_b,
                              GPIO.BOTH, callback=self._b_changed_callback)

    def timer(self) -> None:
        # push button signal is pulled up against VCC;
        pushbutton_signal: bool = not GPIO.input(
            self.gpio_number_signal_pushbutton)
